		...
		var rsPL = {
			...
			secMax: function (jd, L, fa, high) {
				...
				if (this.sT[1] < this.sun_s && this.sf2 > 0) this.sf = this.sf2, this.sflx = "#";
				if (this.sT[1] > this.sun_j && this.sf3 > 0) this.sf = this.sf3, this.sflx = "*";
				for (i = 0; i < 5; i++) {
					if (this.sT[i] < this.sun_s || this.sT[i] > this.sun_j) this.sT[i] = 0;
				}
				this.sun_s -= dt_T(jd);
				this.sun_j -= dt_T(jd);
			},
		...
		function Lunar() {
			...
			this.yueLiHTML = function (By, Bm, curJD) {
				...
				var ta0 = '<tr><td colspan=7 style="background-color:#0000A0;color:#FFFF00">' + c + '</td></tr>';
		...
		var tu1 = {
			...
			showzb: function () {
				...
				this.vs = dx, this.vx = x0 + 4 * dx, this.vy = y0 - 3 * dy;
				this.can.height = this.can.height;
				this.ctx.beginPath();
				this.ctx.fillStyle = '#D0D0D0';
				for (i = 0; i < ditu0.length; i += 2) {
					if (ditu0[i] == 1e7) i++, c = 1; else c = 0;
					x = x0 + 8 * dx * ditu0[i] / 2009;
					y = y0 - 6 * dy * (1 - ditu0[i + 1] / 970);
					if (c) this.ctx.moveTo(x, y); else this.ctx.lineTo(x, y);
				}
				this.ctx.fill();
				this.ctx.beginPath();
			...
			draw2b: function (J1, W1, J2, W2, mr, er, Er) {
				...
				dJ *= Math.cos((W1 + W2) / 2) * rad, dW *= rad;
				dJ = v / 32 * dJ / 60, dW = v / 32 * dW / 60;
				er = v / 32 * er / 60, Er = v / 32 * Er / 60, mr = v / 32 * mr / 60;
				if (Math.abs(dJ) > 3.5 * this.dx || Math.abs(dW) > 2.5 * this.dy) return;
				ht_oval2(this.ctx, this.vx + dJ, this.vy - dW, mr, '#A0A000');
		...
		var tu2 = {
			isInit: 0, eR: 140, x0: 240, y0: 210, init: ht_init, line1: function (as, hd) {
				var i, a, js, xc, yc;
		...
		var tu3 = {
			isInit: 0, eR: 250, x0: 350, y0: 260, init: ht_init, lineArr: function (d, color) {
				var c, x, y;
				d = touY.lineArr(d);
				this.ctx.beginPath();
				...
				if (!(tylx == 3 || tylx == 4 || tylx == 5 || tylx == 6 || jb[2] < 1 || jb[3] < 1)) ht_oval1(this.ctx, this.x0, this.y0, this.eR, '#000000');
			}, draw2: function (F) {
		...
		function tu_calc(ly) { //ly是取时间的方式,xm是计算的项目
			...
			if (msHJ < 3 / radd || msHJ > 357 / radd) { //日食图表放大计算
				...
				//显示南北界数据
				rsPL.nasa_r = 0;
				if (Cb_nasa.checked) rsPL.nasa_r = 1; //视径选择
		...
		function dfRS(ly) { //地方日食表生成
			...
			rsPL.nasa_r = 0;
			if (Cc_nasa.checked) rsPL.nasa_r = 1; //视径选择
			var i, j, t, c,
			...
			for (i = 0; i < s.length; i++) {
				c = s[i];
				if (c.length == 0 || c.substr(0, 1) == '*') continue;
				c = c.split(',');
		...
		function show_clock(t) { //显示时钟,传入日期对象
			...
			var mm = JD.m < 10 ? '0' + JD.m : JD.m; //分格式转为88
			var ss = int2(JD.s) < 10 ? '0' + int2(JD.s) : int2(JD.s); //秒格式转为88

			//Clock2.innerHTML = JD.D+'日 '+JD.h+':'+JD.m+':'+int2(JD.s) + rg; //为了与clock1同步,秒数取整而不四舍五入
			Clock2.innerHTML = h + '时区　　' + JD.D + '日 ' + JD.h + ':' + mm + ':' + ss + rg; //为了与clock1同步,秒数取整而不四舍五入
		}
		...
		function bdptys(secs, J, Gs) { //本地平太阳时计算。secs为本日过去的秒数,J为本地经度.
			var bzjd0 = 120; //标准时间发出地经度
			var ptys = int2(secs + 4 * (J - bzjd0) * 60); //平太阳时,每经度时差4分钟
			var ptysds = ptys / 3600 / 24; //平太阳时化为日数
			if (ptys < 0) ptys = 24 * 3600 + ptys; //负值时转为前一日
			var ptysh = int2(ptys / 3600); //平小时
			var ptysm = int2((ptys - ptysh * 3600) / 60); //平分钟
			var ptyss = int2(ptys - ptysh * 3600 - ptysm * 60); //平秒
			if (ptysh < 10) ptysh = '0' + ptysh; //平小时
			if (ptysm < 10) ptysm = '0' + ptysm; //平分钟
			if (ptyss < 10) ptyss = '0' + ptyss; //平秒
			if (Gs == 'd') {
				return ptysds;
			} else {
				return ptysh + ':' + ptysm + ':' + ptyss; //返回平太阳时串
			}
		}
		...
		function showMessD(n) { //显时本月第n日的摘要信息。调用前应先执月历页面生成，产生有效的lun对象
			...
			if (window.event && window.event.srcElement.tagName == 'SPAN') { //鼠标移过日期上方
				...
				// Cal_pan.style.left = "700px";
				// Cal_pan.style.top = "300px";
				Cal_pan.style.left = window.event.x + document.body.scrollLeft + ((ob.week > 3) ? -180 : 20) + "px";
				Cal_pan.style.top = 300 + window.event.y + document.body.scrollTop - ((ob.weeki < 2) ? 0 : 100) + "px";
			}
		}
		...
		function dingQi_v() { //定气计算速度测试
			var d1 = new Date();
			for (i = 0; i < 1000; i++) XL.S_aLon_t(0);
			var d2 = new Date();
			for (i = 0; i < 1000; i++) XL.S_aLon_t2(0);
			var d3 = new Date();
